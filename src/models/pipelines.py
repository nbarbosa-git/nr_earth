...
def get_criteria(model, importances_df):
    ...
    if model[:15]=='metric/ar1_sign': criteria = importances_df['mean'].quantile(1-.1)

    if model[:25]=='dss/cluster/all_feats/mda': criteria = 0

    #reverse criteria <,>
    if model[:15]=='metric/ar1_sign': importances_df = importances_df[importances_df['mean']<criteria]
    else: importances_df = importances_df[importances_df['mean']>criteria]
...
def get_features(model_path, features_sort, ranker=False):
    ...
    #'https://raw.githubusercontent.com/nbarbosa-git/dissertacao/master/reports/feature_importance/linear/mda_linear_reg.csv'
