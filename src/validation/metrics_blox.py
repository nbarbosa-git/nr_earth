...
from metrics_tools import correlation_contribution
...
BENCH_METRICS = ["corr_with", "mc_mean_std_sharpe", "legacy_mc_mean_std_sharpe",
                 #"ex_diss", "ex_diss_pearson", "ex_diss_spearman"
                 ]
    ...
    def plot_correlations(
        ...
        if payout=='MMC':
          ...
          for pred_col in tqdm(pred_cols):
              ...
              per_era_corrs = pd.Series([i.values[0] for i in per_era_corrs], index=dataf.era.unique())
              validation_by_eras.loc[:, pred_col] = per_era_corrs
        ...
        if payout=='CORRMMC':
          ...
          for pred_col in tqdm(pred_cols):
              ...
              corr_validation_by_eras.loc[:, pred_col] = per_era_corrs
          ...
          for pred_col in tqdm(pred_cols):
              ...
              per_era_corrs = pd.Series([i.values[0] for i in per_era_corrs], index=dataf.era.unique())
              mmc_validation_by_eras.loc[:, pred_col] = per_era_corrs
          validation_by_eras=0.5*corr_validation_by_eras+2*mmc_validation_by_eras
        #return validation_by_eras, corr_validation_by_eras, mmc_validation_by_eras
        ...
        validation_by_eras.rolling(roll_mean).mean().plot(
            ...
            marker="o",
            ms=4,
            title=f"Rolling Per Era {payout} Mean (rolling window size: {roll_mean})",
            figsize=(15, 5),
        )
        plt.legend(
            loc="upper center",
        ...
        validation_by_eras.cumsum().plot(
            title=f"Cumulative Sum of Era {payout}", figsize=(15, 5)
        )
        plt.legend(
            loc="upper center",
            bbox_to_anchor=(0.5, -0.05),
