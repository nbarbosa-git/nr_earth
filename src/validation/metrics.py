...
def smart_sharpe(x):
    return (np.mean(x)/(np.std(x) * autocorr_penalty(x))) #* np.sqrt(12)) 


def smart_sharpe_abs(x): ##Suitable to one feat analysis
    return (abs(np.mean(x))/(np.std(x) * autocorr_penalty(x, sign=True))) #* np.sqrt(12)) 
...
def sortino_ratio(x, target=0.0):
    xt = x - target
    return np.mean(xt) / (np.sum(np.minimum(0, xt)**2)/(len(xt)-1))**.5


#OK
def smart_sortino_ratio(x, target=0.0): ##approximated their average trading costs 010415154
    xt = x - target
...
def annual_payout(scores):

        payout_scores = scores.clip(lower=-0.25, upper=0.25)
...
def get_feature_neutral_mean(ddf, preds): #ORIG
    df = ddf.copy()
...
def mmc_metrics(df, preds, model):
    ...
    elif df.shape[0] == 539658: 
        file = '_predictions.csv'
        path= 'predicoes_legacy'
        model='gbmsnr_15'
    ...
    file_path = 'https://raw.githubusercontent.com/nbarbosa-git/compute_info/main/public_semanal/'+path+'/raw/'+model+file
    #print(file_path)
    ...
    corr_scores = pd.Series(corr_scores, index=df['era'].unique())
    return val_mmc_mean, corr_plus_mmc_sharpe, corr_with_example_preds, mmc_scores, corr_scores #val_mmc_sharpe
...
def tb200_stats(df, columns, target, tb=None, era_col="era"):

    validation_data = df.copy()
    features = [x for x in validation_data.columns if x.startswith('feature_')]
    ...
    tb200_mean = tb200_validation_correlations.mean()[PREDICTION_NAME]
    tb200_std = tb200_validation_correlations.std(ddof=0)[PREDICTION_NAME]
    tb200_sharpe = tb200_mean / tb200_std
    tb200_scores = tb200_validation_correlations[PREDICTION_NAME]
    ...
    tb200_max_exp = tb200_validation_correlations[features[:]].T.abs().max().mean()
    return tb200_scores, tb200_mean, tb200_std, tb200_sharpe, tb200_validation_correlations[features[:]], tb200_max_exp
...
def submission_metrics(df_val, preds, model_name, full=True, meta=''):
    ...
    if full:
        ...
        values['Feat_neutral_mean'] = get_feature_neutral_mean(new_df, preds)
        values['Feat_exp_std'], values['Feat_exp_max'], feat_corrs  = feature_exposure(new_df, preds)
        values['val_mmc_mean'], values['corr_plus_mmc_sharpe'], values['corr_with_example_preds'], mmc_scores, values['val_mmc_sharpe'] = mmc_metrics(new_df, preds, 'ex_preds')
        values['FNC'] = calculate_fnc(pd.Series(preds, index=new_df.index), new_df.target, new_df[features])


        tb200_scores, values['tb200_mean'], values['tb200_std'], values['tb200_sharpe'], tb200_exp, values['tb200_max_exp_mean'] = tb200_stats(new_df,[PREDICTION_NAME],'target', tb=200, era_col="era")
        ...
        values['Smart_Sharpe_Abs'] = smart_sharpe_abs(era_scores)
        ...
        values['Payout'] = payout(era_scores+values['val_mmc_mean'])
        values['Annual_Payout'] = annual_payout(era_scores)
        ...
        values['FNC'] = 0
        tb200_scores, values['tb200_mean'], values['tb200_std'], values['tb200_sharpe'], tb200_exp, values['tb200_max_exp_mean'] = 0,0,0,0,feat_corrs,0
    ...
    return era_scores, df_metrics, feat_corrs, mmc_scores, tb200_scores, tb200_exp
